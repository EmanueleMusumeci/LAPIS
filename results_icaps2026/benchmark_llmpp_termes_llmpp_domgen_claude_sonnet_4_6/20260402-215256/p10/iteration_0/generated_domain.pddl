(define (domain block-building)
  ...
  (:action place
    :parameters (?robot-pos - position ?block-pos - position ?h-cur - height ?h-new - height)
    :precondition (and
      (at ?robot-pos)
      (neighboring ?robot-pos ?block-pos)
      (has-block)
      (height-at ?robot-pos ?h-cur)
      (height-at ?block-pos ?h-cur)
      (not (is-depot ?block-pos))
      (next-height ?h-cur ?h-new)
    )
    :effect (and
      (not (has-block))
      (not (height-at ?block-pos ?h-cur))
