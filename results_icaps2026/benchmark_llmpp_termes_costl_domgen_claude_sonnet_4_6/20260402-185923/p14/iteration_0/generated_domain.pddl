(define (domain blocksworld-robot)
  ...
  ;; Move horizontally between two positions at the same height
  (:action move
  ...
  ;; Move up from a lower position to a higher neighboring position
  (:action move-up
  ...
  ;; Move down from a higher position to a lower neighboring position
  (:action move-down
    :parameters (?from - position ?to - position ?h-low - height ?h-high - height)
    :precondition (and
      (at ?from)
