...
# ── inject CSS ────────────────────────────────────────────────────────────────
_css_path = _DEMO_DIR / "style_premium.css"
if _css_path.exists():
...
def _run_buttons(domain_nl: str, problem_nl: str):
    ...
    run_race  = col_c.button("🏁 MODEL RACE", disabled=st.session_state.running_lapis or st.session_state.running_llmpp, use_container_width=True)
    return run_lapis, run_llmpp, run_race

def _execute_run_parallel(domain_nl: str, problem_nl: str, containers):
    """Run LAPIS and LLM+P in parallel threads."""
    import threading
    ...
    def run_wrapper(method):
        ...
        def on_update(sr: StageResult):
            stages_key = f"stages_{method}"
            stages = st.session_state[stages_key]
            existing = [s.name for s in stages]
            if sr.name in existing:
                idx = existing.index(sr.name)
                stages[idx] = sr
            else:
                stages.append(sr)
            # Render to specific placeholder
            ph = lapis_ph if method == "lapis" else llmpp_ph
            with ph.container():
    ...
    progress_bar = st.progress(0, text="🏁 Racing models in parallel...")
    start_t = time.time()
    
    while t1.is_alive() or t2.is_alive():
        # Update progress bar based on stages completed (max 4 per model)
        s1 = len(st.session_state.stages_lapis)
        s2 = len(st.session_state.stages_llmpp)
        progress = min(0.99, (s1 + s2) / 8.0)
        progress_bar.progress(progress, text=f"🏁 Race in progress... ({time.time() - start_t:.1f}s)")
        
        # Explicit live update from main thread to placeholders
        with lapis_ph.container():
            st.markdown("#### LAPIS 🌀")
            _render_all_stage_cards(st.session_state.stages_lapis)
        with llmpp_ph.container():
            st.markdown("#### LLM+P 🚀")
...
def main():
    ...
    run_lapis, run_llmpp, run_race = _run_buttons(domain_nl, problem_nl)
    ...
    if run_race:
        print("DEBUG: run_race clicked")
        if not domain_nl.strip() or not problem_nl.strip():
            st.error("Please provide both descriptions for the race.")
        else:
            with stages_container:
                st.markdown("### 🏁 THE MODEL RACE")
                c_lapis, c_llmpp = st.columns(2)
                lapis_ph = c_lapis.empty()
                llmpp_ph = c_llmpp.empty()
            
            _execute_run_parallel(domain_nl, problem_nl, (lapis_ph, llmpp_ph))
            st.rerun()
