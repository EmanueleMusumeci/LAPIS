...
def trace_check(trace, formula_str, constraint_evaluation_mode="spot"):
    ...
    # Trace back to CoSTL/ (6 levels up from src/costl/planner/high/Planner/)
    # Or just find the root dynamically
    costl_root = current_dir
    while os.path.basename(costl_root) != "CoSTL" and costl_root != "/":
        costl_root = os.path.dirname(costl_root)
