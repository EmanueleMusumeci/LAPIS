"""
LexiconLowLevelPipeline — CoSTL low-level planner evaluated on Lexicon benchmark problems.

Reads the NL task description from the `nl` file in each problem folder, uses the
pre-existing `domain.pddl`, runs the CoSTL problem-generation + planning + refinement
...
from src.costl.pipelines.low_level_planning import LowLevelPlanningPipeline
from src.costl.planner.low.pddl_verification import VAL_validate
from src.costl.plan_renderer import simulate_plan, render_plan_gif
from src.costl.logger_cfg import logger


class LexiconLowLevelPipeline(LowLevelPlanningPipeline):
    """
    ...
    def _process_task(self, problem_id: str, results_dir: str):
        ...
        if self.generate_domain and self.llmpp_source_dir:
            ...
            if not domain_file.exists():
                ...
                return
            # Run CoSTL low-level planning loop
