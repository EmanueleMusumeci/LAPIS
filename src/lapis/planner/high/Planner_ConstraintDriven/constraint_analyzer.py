...
class ConstraintAnalyzerDescription(BaseModel):
    reasoning: List[str]
    new_post_conditions: Optional[List[str]]    


class ConstraintAnalyzerAgent(GPTAgent):
    ...
    with a pydantic `response_format` model (e.g., `ConstraintAnalyzerDescription`).
    """
    def __init__(self, model: str, system_prompt: Optional[str] = None, prompt_file: Optional[str] = None, max_history: int = 1):
        ...
        prompt_path = os.path.join(os.path.dirname(__file__), "Prompts", "constraint_analyzer_prompt.txt")
        try:
            with open(prompt_path, "r", encoding="utf-8") as pf:
                self.system_prompt = pf.read().strip()
        except Exception:
            print('ERROR: Could not load system prompt from file for ConstraintAnalyzerAgent.')
...
def constraint_analysis(domain, problem, states, plan, goal, conditional_constraints, global_constraints, agent):
    ...
    States: {states}
    Plan so far: {plan}
    Conditional Statement : {conditional_constraints}
    """

    response = agent.step(user_input, response_model=ConstraintAnalyzerDescription)
    description = response.choices[0].message.parsed

    reasoning = description.reasoning
    new_post_conditions = description.new_post_conditions

    print('######################################')
    print('ORIGINAL DOMAIN: ', domain)
    print('ORIGINAL PROBLEM: ', problem)
    print('STATES: ', states)
    print('PLAN SO FAR: ', plan)
    print('CONDITIONAL CONSTRAINTS: ', conditional_constraints)
    print('GLOBAL CONSTRAINTS: ', global_constraints)
    print('GOAL: ', goal)
    print('\nREASONING: ', reasoning)
    print('\nNEW POST-CONDITIONS (if any): ', new_post_conditions)
    ...
    return reasoning, new_post_conditions