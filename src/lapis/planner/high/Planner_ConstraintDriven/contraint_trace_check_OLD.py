...
class ConstraintCheckDescription(BaseModel):
    reasoning: List[str]
    satisfied: List[bool]
    how_to_solve: Optional[List[str]]
    

class ConstraintCheckSelector(GPTAgent):
    ...
    def __init__(self, model: str, system_prompt: Optional[str] = None, prompt_file: Optional[str] = None, max_history: int = 1):
        ...
        prompt_path = os.path.join(os.path.dirname(__file__), "Prompts", "constraint_trace_check_prompt.txt")
        try:
            with open(prompt_path, "r", encoding="utf-8") as pf:
                self.system_prompt = pf.read().strip()
        except Exception:
            print('ERROR: Could not load system prompt from file for ConstraintCheckSelector.')
...
def constraint_check(domain, problem, states, plan, goal, constraints, conditional_constraints, global_constraints, agent):
    ...
    response = agent.step(user_input, response_model=ConstraintCheckDescription)
    description = response.choices[0].message.parsed

    reasoning = description.reasoning
    satisfied = description.satisfied
    how_to_solve = description.how_to_solve

    return reasoning, satisfied, how_to_solve