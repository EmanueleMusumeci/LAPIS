(define (domain liftedtcore_minigrid-domain)
 ...
 (:constants
   empty - empty_0
   purple_door_1 yellow_door_1 - door
   room_3 - room
   red_box_1 - box
 )
 ...
 (not (blocked ?door ?o ?room1))) (or (not (= ?door yellow_door_1)) (seen_psi_3)))
  :effect (and(forall (?o - object_) (when (at_ ?o) (not (at_ ?o))))(forall (?d - door) (when (at_ ?d) (not (at_ ?d)))) (at_ ?door) (when (= ?door yellow_door_1) (hold_2)) (increase (total-cost) 1)))
 (:action gotoroom
 ...
 (:action pick
  ...
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (hold_0) (when (or (= ?obj red_box_1) (carrying red_box_1)) (hold_1)) (seen_psi_3) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (carrying ?obj) (not (emptyhands)) (at_ empty))
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (and (carrying red_box_1) (not (= ?obj red_box_1))) (hold_1)) (increase (total-cost) 1)))
 (:action toggle
