(define (domain liftedtcore_minigrid-domain)
 ...
 (:constants
   empty - empty_0
   room_2 - room
   green_door_2 green_door_1 - door
   blue_ball_1 - ball
 )
 ...
 (:action pick
  ...
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (hold_0) (when (and (not (locked green_door_2)) (not (or (= ?obj blue_ball_1) (carrying blue_ball_1)))) (not (hold_2))) (when (or (= ?obj blue_ball_1) (carrying blue_ball_1)) (hold_2)) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (carrying ?obj) (not (emptyhands)) (at_ empty))
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (or (and (= ?obj blue_ball_1) (= ?room room_2)) (objectinroom blue_ball_1 room_2)) (hold_0)) (when (and (not (locked green_door_2)) (not (and (carrying blue_ball_1) (not (= ?obj blue_ball_1))))) (not (hold_2))) (when (and (carrying blue_ball_1) (not (= ?obj blue_ball_1))) (hold_2)) (increase (total-cost) 1)))
 (:action toggle
  ...
  :effect (and (when (not (locked ?door)) (locked ?door)) (when (locked ?door) (not (locked ?door))) (when (not (or (and (not (locked ?door)) (= ?door green_door_2)) (and (locked green_door_2) (not (and (locked ?door) (= ?door green_door_2)))))) (hold_1)) (when (and (not (or (and (not (locked ?door)) (= ?door green_door_2)) (and (locked green_door_2) (not (and (locked ?door) (= ?door green_door_2)))))) (not (carrying blue_ball_1))) (not (hold_2))) (increase (total-cost) 1)))
)
