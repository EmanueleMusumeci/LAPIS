(define (domain liftedtcore_minigrid-domain)
 ...
 (:action gotoobject
  ...
  :effect (and(forall (?o - object_) (when (at_ ?o) (not (at_ ?o))))(forall (?d - door) (when (at_ ?d) (not (at_ ?d)))) (at_ ?obj) (when (and (objectinroom red_ball_1 room_4) (= ?obj purple_ball_1)) (hold_0)) (increase (total-cost) 1)))
 (:action gotoempty
  :parameters ()
 ...
 (:action pick
  ...
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (when (and (objectinroom red_ball_1 room_4) (not (and (= ?obj red_ball_1) (= ?room room_4))) (at_ purple_ball_1) (not (= ?obj purple_ball_1))) (hold_0)) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (carrying ?obj) (not (emptyhands)) (at_ empty))
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (and (or (and (= ?obj red_ball_1) (= ?room room_4)) (objectinroom red_ball_1 room_4)) (or (= ?obj purple_ball_1) (at_ purple_ball_1))) (hold_0)) (increase (total-cost) 1)))
 (:action toggle
