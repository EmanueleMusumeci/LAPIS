(define (domain build-structures)
  (:requirements :strips :typing :conditional-effects)

  (:types
    position
    height
  )

  (:predicates
    (at ?r - position)
    (neighboring ?p1 - position ?p2 - position)
    (height-at ?p - position ?h - height)
    (next-height ?h1 - height ?h2 - height)
    (has-block)
    (is-depot ?p - position)
  )

  (:action move
    :parameters (?from - position ?to - position ?h - height)
    :precondition (and
      (at ?from)
      (neighboring ?from ?to)
      (height-at ?from ?h)
      (height-at ?to ?h)
    )
    :effect (and
      (not (at ?from))
      (at ?to)
    )
  )

  (:action move-up
    :parameters (?from - position ?to - position ?h-from - height ?h-to - height)
    :precondition (and
      (at ?from)
      (neighboring ?from ?to)
      (height-at ?from ?h-from)
      (height-at ?to ?h-to)
      (next-height ?h-from ?h-to)
    )
    :effect (and
      (not (at ?from))
      (at ?to)
    )
  )

  (:action move-down
    :parameters (?from - position ?to - position ?h-from - height ?h-to - height)
    :precondition (and
      (at ?from)
      (neighboring ?from ?to)
      (height-at ?from ?h-from)
      (height-at ?to ?h-to)
      (next-height ?h-to ?h-from)
    )
    :effect (and
      (not (at ?from))
      (at ?to)
    )
  )

  (:action place
    :parameters (?pos - position ?block-pos - position ?h - height ?h-new - height)
    :precondition (and
      (at ?pos)
      (neighboring ?pos ?block-pos)
      (has-block)
      (height-at ?pos ?h)
      (height-at ?block-pos ?h)
      (not (is-depot ?block-pos))
      (next-height ?h ?h-new)
    )
    :effect (and
      (not (has-block))
      (not (height-at ?block-pos ?h))
      (height-at ?block-pos ?h-new)
    )
  )

  (:action remove
    :parameters (?pos - position ?block-pos - position ?h - height ?h-new - height)
    :precondition (and
      (at ?pos)
      (neighboring ?pos ?block-pos)
      (not (has-block))
      (not (is-depot ?block-pos))
      (height-at ?pos ?h)
      (height-at ?block-pos ?h)
      (next-height ?h-new ?h)
    )
    :effect (and
      (has-block)
      (not (height-at ?block-pos ?h))
      (height-at ?block-pos ?h-new)
    )
  )

  (:action create-block
    :parameters (?pos - position)
    :precondition (and
      (at ?pos)
      (is-depot ?pos)
      (not (has-block))
    )
    :effect (and
      (has-block)
    )
  )

  (:action destroy-block
    :parameters (?pos - position)
    :precondition (and
      (at ?pos)
      (is-depot ?pos)
      (has-block)
    )
    :effect (and
      (not (has-block))
    )
  )

)